...
class ShippingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create shipping" do
    assert_difference('Shipping.count') do
      post shippings_url, params: { shipping: { key: @shipping.key, references: @shipping.references, status: @shipping.status } }, as: :json
    end
  ...
  test "should update shipping" do
    patch shipping_url(@shipping), params: { shipping: { key: @shipping.key, references: @shipping.references, status: @shipping.status } }, as: :json
    assert_response 200
  end
